#### V2 - Added skip for objects being detected that are not relevant
...
def save_mesh_assignments_json(
    ...
    data_root: Path,
    timestamp: str,
    mesh_by_object: Dict[str, Optional[Path]],
    ...
    for object_name, item in objects_payload.items():
        if item.get("skip"):
            print(f"  - {object_name}: skipped=True, mesh_id=None, mesh=None")
            continue
...
def prompt_mesh_for_object(
    ...
    while True:
        ...
        if available_meshes:
            ...
            print("  0. PASS / ignore this object")
            print("Insert a number from the list, insert 0/pass to skip, or insert a mesh path.")
        else:
            print(f"No meshes found under: {data_root / 'meshes'}")
            print("Insert 0/pass to skip this object, an absolute mesh path, or a path relative to data_root.")

        user_input = input(f"CAD for {object_name}: ").strip()
        normalized_input = user_input.lower()

        if normalized_input in {"0", "p", "pass", "skip", "ignore"}:
            print(f"[FP] Skipping object {object_name}: no CAD mesh assigned.")
            return None
...
def main():
    ...
    for object_name, first_mask_path in object_masks:
        ...
        if mesh_path is None:
            ...
            print("[FP] Mesh: PASS / skipped")
            print("[FP] Skipping FoundationPose for this object.")
            print("======================================================")
            continue
        ...
        mesh, to_origin, _extents, bbox = load_mesh_geometry(mesh_path)
...
#### V1 - First version after pipeline on real robot
